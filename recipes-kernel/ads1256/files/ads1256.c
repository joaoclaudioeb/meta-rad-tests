...
enum ads1256_bitorder { ADS1256_BITORDER_MSB = 0, ADS1256_BITORDER_LSB = 1 };

enum ads1256_acal { ADS1256_ACAL_DISABLED = 0, ADS1256_ACAL_ENABLED = 1 };

enum ads1256_buffer { ADS1256_BUFFER_DISABLED = 0, ADS1256_BUFFER_ENABLED = 1 };
...
struct ads1256_private {
	...
	u32 buffer[ADS1256_MAX_CHANNELS + sizeof(s64) / sizeof(u32)] __aligned(
		8);
...
static int ads1256_read(struct iio_dev *indio_dev, int *result)
{
	...
	struct spi_transfer t[2] = {
		{
			.tx_buf = &priv->data[0],
			.len = 1,
			.delay = { .value = 7, /* t6 delay (datasheet) */
				   .unit = SPI_DELAY_UNIT_USECS },
		},
		{
			.tx_buf = &priv->data[1],
	...
	dev_dbg(&priv->spi->dev, "RAW bytes = %02x %02x %02x\n", priv->data[3],
		priv->data[4], priv->data[5]);

	val = ((int32_t)priv->data[1] << 16) | ((int32_t)priv->data[2] << 8) |
	      (int32_t)priv->data[3];
			    ...
			    int *val2, long mask)
{
	...
	case IIO_CHAN_INFO_RAW:
		ret = ads1256_set_channel(
			indio_dev, ads1256_mux_ch_table[chan->scan_index]);
		if (ret)
			goto out;

		ret = ads1256_write_reg(indio_dev, ADS1256_REG_MUX,
					ads1256_mux_table[chan->scan_index]);
		if (ret)
			goto out;
...
static int ads1256_probe(struct spi_device *spi)
{
	...
	/* Request SYNC GPIO */
	priv->sync_gpio =
		devm_gpiod_get_optional(&spi->dev, "sync", GPIOD_OUT_HIGH);
	...
	if (ret) {
		priv->status_reg =
			(ADS1256_BUFFER_DISABLED << 1) |
			(ADS1256_ACAL_DISABLED << 2) |
			(ADS1256_BITORDER_MSB
			 << 3); /* STATUS (default): BUFEN=0, ACAL=0, ORDER=MSB */
		dev_dbg(&spi->dev, "ti,status not found, using default 0x%x\n",
			priv->status_reg);
	} else {
		priv->status_reg = tmp & 0xff;
	}
	ret = ads1256_write_reg(indio_dev, ADS1256_REG_STATUS,
				priv->status_reg);
	if (ret)
		return ret;
	...
	if (ret) {
		priv->adcon_reg =
			ADS1256_PGA_1; /* ADCON (default): PGA=1, CLKOUT=off, SDCS=off */
	...
	if (ret) {
		priv->drate_reg =
			ADS1256_DRATE_10SPS; /* DRATE (default): 10 SPS */
		dev_dbg(&spi->dev,
			"ti,drate not found, using default 10 SPS\n");
	} else {
...
static const struct spi_device_id ads1256_id[] = { { "ads1256", 0 }, {} };
